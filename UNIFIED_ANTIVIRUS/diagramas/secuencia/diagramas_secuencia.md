...
## 🔄 Actualización de Firmas y Modelos

```plantuml
@startuml secuencia_actualizacion_firmas
title Actualización de Firmas y Modelos ML

participant "Update Scheduler" as scheduler
participant "Update Manager" as manager
participant "Central Server" as server
participant "Integrity Checker" as checker
participant "Model Deployer" as deployer
participant "Detector Engines" as detectors
participant "Backup System" as backup
participant "Notification Service" as notifications

== Verificación Programada ==
scheduler -> manager : check_for_updates()
activate manager

manager -> server : query_available_updates()
activate server
server -> manager : update_manifest[]
deactivate server

manager -> manager : compare_local_versions()

alt Updates Available
    == Descarga de Actualizaciones ==
    loop Para cada actualización
        manager -> server : download_update(update_id)
        activate server
        server -> manager : update_package
        deactivate server
        
        manager -> checker : verify_integrity(package, signature)
        activate checker
        checker -> checker : validate_checksum()
        checker -> checker : verify_digital_signature()
        checker -> manager : integrity_verified
        deactivate checker
        
        alt Integrity Valid
            manager -> backup : create_rollback_point()
            activate backup
            backup -> manager : backup_created
            deactivate backup
        else Integrity Failed
            manager -> server : report_corruption(update_id)
            manager -> manager : try_alternative_source()
        end
    end
    
    == Aplicación de Actualizaciones ==
    manager -> deployer : deploy_updates(validated_packages)
    activate deployer
    
    deployer -> deployer : extract_packages()
    deployer -> deployer : prepare_deployment()
    
    alt Signature Updates
        deployer -> detectors : update_signatures(new_signatures)
        activate detectors
        detectors -> deployer : signatures_loaded
        deactivate detectors
        
    else ML Model Updates
        deployer -> detectors : update_ml_models(new_models)
        activate detectors
        detectors -> detectors : load_new_models()
        detectors -> detectors : validate_model_performance()
        detectors -> deployer : models_deployed
        deactivate detectors
        
    else Configuration Updates
        deployer -> detectors : update_configurations(new_configs)
        activate detectors
        detectors -> deployer : configs_applied
        deactivate detectors
    end
    
    deployer -> manager : deployment_completed
    deactivate deployer
    
    == Verificación Post-Update ==
    manager -> detectors : run_health_check()
    activate detectors
    detectors -> manager : health_status
    deactivate detectors
    
    alt Health Check Passed
        manager -> notifications : send_update_success()
        activate notifications
        notifications -> manager : notification_sent
        deactivate notifications
        
    else Health Check Failed
        manager -> backup : rollback_to_previous_version()
        activate backup
        backup -> manager : rollback_completed
        deactivate backup
        
        manager -> notifications : send_rollback_notification()
        activate notifications
        notifications -> manager : rollback_notification_sent
        deactivate notifications
    end
    
else No Updates
    manager -> scheduler : schedule_next_check()
end
