...
class TestAPIHookingDetectionTDD:
    ...
    def test_detect_hooking_apis_should_return_high_risk(self, keylogger_detector):
        ...
        assert result['risk_score'] >= 0.8, f"Score debe ser alto (>=0.8), got {result['risk_score']}"
        assert 'api_hooking' in result['threat_indicators'], "Debe identificar hooking como amenaza"
        assert len(result['suspicious_apis']) >= 3, "Debe detectar las 3 APIs críticas"
        
    @pytest.mark.tdd
    @pytest.mark.api_detection  
    def test_detect_file_logging_apis_should_return_medium_risk(self, keylogger_detector):
        """
        ...
        assert 0.4 <= result['risk_score'] < 0.8, f"Score medio esperado [0.4, 0.8), got {result['risk_score']}"
        assert 'file_logging' in result['threat_indicators'], "Debe identificar logging como sospechoso"
    ...
    def test_legitimate_apis_should_return_low_risk(self, keylogger_detector):
        ...
        assert result['is_suspicious'] is False, "APIs legítimas no deben ser sospechosas"
    ...
    def test_mixed_apis_should_calculate_weighted_score(self, keylogger_detector):
        ...
        assert 0.6 <= result['risk_score'] < 0.9, f"Score ponderado esperado [0.6, 0.9), got {result['risk_score']}"
        assert 'api_hooking' in result['threat_indicators'], "Debe detectar hooking"
        assert 'legitimate_usage' in result['details'], "Debe reconocer uso legítimo también"
