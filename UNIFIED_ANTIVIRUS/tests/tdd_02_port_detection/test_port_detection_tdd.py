...
class TestSuspiciousPortDetectionTDD:
    ...
    def test_suspicious_port_4444_should_be_flagged(self, network_detector):
        ...
        assert result['is_suspicious'] is True, "Puerto 4444 debe ser detectado como sospechoso"
        assert result['risk_score'] >= 0.8, f"Score alto esperado (>=0.8), got {result['risk_score']}"
        assert 4444 in result['suspicious_ports'], "Puerto 4444 debe estar en lista sospechosa"
        assert 'c2_communication' in result['threat_indicators'], "Debe identificar comunicación C&C"
    ...
    def test_legitimate_https_port_443_should_pass(self, network_detector):
        ...
        assert result['risk_score'] < 0.3, f"Score bajo esperado (<0.3), got {result['risk_score']}"
        assert 443 not in result.get('suspicious_ports', []), "Puerto 443 no debe estar en lista sospechosa"
        assert len(result['threat_indicators']) == 0, "No debe haber indicadores de amenaza"
    ...
    def test_port_classification_accuracy(self, network_detector, port, expected_risk):
        ...
        if expected_risk == 'high':
            assert result['is_suspicious'] is True, f"Puerto {port} debe ser de alto riesgo"
            assert result['risk_score'] >= 0.8, f"Score alto para puerto {port}"
        elif expected_risk == 'medium':
            assert result['is_suspicious'] is True, f"Puerto {port} debe ser de medio riesgo" 
            assert 0.4 <= result['risk_score'] < 0.8, f"Score medio para puerto {port}"
        else:  # low risk
            assert result['is_suspicious'] is False, f"Puerto {port} debe ser de bajo riesgo"
    ...
    def test_multiple_suspicious_connections_aggregate_risk(self, network_detector):
        ...
        network_data = {
            ...
            'connections': [
                {
                    'remote_port': 1337,  # High risk
                    'state': 'ESTABLISHED'
                },
                {
                    'remote_port': 4444,  # High risk  
                    'state': 'ESTABLISHED'
                },
                {
                    'remote_port': 8080,  # Medium risk
                    'state': 'ESTABLISHED'
                }
            ]
        }
        ...
        assert result['is_suspicious'] is True, "Múltiples puertos sospechosos deben alertar"
        assert result['risk_score'] >= 0.9, f"Score muy alto para múltiples puertos, got {result['risk_score']}"
        assert len(result['suspicious_ports']) == 3, "Debe detectar los 3 puertos sospechosos"
        assert 'multiple_suspicious_ports' in result['threat_indicators'], "Debe detectar patrón múltiple"
    ...
    def test_connection_frequency_analysis(self, network_detector):
        ...
        assert result['is_suspicious'] is True, "Conexiones frecuentes deben ser sospechosas"
        assert 'beaconing_pattern' in result['threat_indicators'], "Debe detectar patrón de beaconing"
        assert result['connection_frequency'] >= 0.2, "Debe calcular frecuencia alta (>0.2 conn/sec)"
    ...
    def test_no_connections_should_return_neutral(self, network_detector):
        ...
        assert result['risk_score'] == 0.0, f"Score neutral esperado (0.0), got {result['risk_score']}"
