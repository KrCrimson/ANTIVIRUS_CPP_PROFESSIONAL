...
class TestSafeProcessValidationTDD:
    ...
    def test_notepad_should_be_validated_as_safe(self, process_validator):
        ...
        assert result['confidence'] >= 0.9, f"Confianza alta esperada (>=0.9), got {result['confidence']}"
        assert result['threat_score'] == 0.0, f"Sin amenaza esperado (0.0), got {result['threat_score']}"
        assert result['category'] == 'system_process', "Debe ser categorizado como proceso del sistema"
    ...
    def test_chrome_browser_should_be_validated_as_safe(self, process_validator):
        ...
        assert result['confidence'] >= 0.85, f"Confianza alta para Chrome, got {result['confidence']}"
    ...
    def test_obvious_keylogger_should_be_flagged(self, process_validator):
        ...
        assert result['is_safe'] is False, "Keylogger.exe debe ser flagueado como peligroso"
        assert result['confidence'] <= 0.1, f"Confianza muy baja esperada (<=0.1), got {result['confidence']}"
        assert result['threat_score'] >= 0.9, f"Score alto de amenaza esperado (>=0.9), got {result['threat_score']}"
        assert 'obvious_malware' in result['threat_indicators'], "Debe detectar malware obvio"
        
    @pytest.mark.tdd
    @pytest.mark.safe_process
    @pytest.mark.parametrize("process_name,expected_safe,category", [
    ...
    def test_process_categorization_accuracy(self, process_validator, process_name, expected_safe, category):
        """
        ...
        assert result['is_safe'] == expected_safe, f"{process_name} clasificación incorrecta"
        
        if expected_safe:
            assert result['confidence'] >= 0.7, f"Confianza alta para proceso seguro {process_name}"
            assert result['threat_score'] <= 0.2, f"Score bajo de amenaza para {process_name}"
        else:
            assert result['confidence'] <= 0.3, f"Confianza baja para proceso peligroso {process_name}"  
            assert result['threat_score'] >= 0.7, f"Score alto de amenaza para {process_name}"
    ...
    def test_suspicious_location_should_lower_trust(self, process_validator):
        ...
        assert result['is_safe'] is False, "Notepad en ubicación sospechosa debe ser flagueado"
        assert result['confidence'] <= 0.5, f"Confianza reducida por ubicación, got {result['confidence']}"
        assert 'suspicious_location' in result['threat_indicators'], "Debe detectar ubicación sospechosa"
        assert 'impersonation_attempt' in result['threat_indicators'], "Puede ser intento de suplantación"
    ...
    def test_unknown_process_should_require_investigation(self, process_validator):
        ...
        assert result['is_safe'] is None, "Proceso desconocido debe tener estado neutral"
        assert 0.3 <= result['confidence'] <= 0.7, f"Confianza neutral para desconocido, got {result['confidence']}"
        assert result['requires_investigation'] is True, "Debe requerir investigación adicional"
    ...
    def test_digital_signature_validation(self, process_validator):
        ...
        assert signed_result['confidence'] > unsigned_result['confidence'], "Firma debe aumentar confianza"
        assert signed_result['threat_score'] < unsigned_result['threat_score'], "Firma debe reducir amenaza"
        assert 'trusted_signature' in signed_result['trust_factors'], "Debe reconocer firma confiable"