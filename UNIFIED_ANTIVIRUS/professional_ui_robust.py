...
class RobustAntivirusUI:
    ...
    def __init__(self):
        ...
        self.update_interval = 5.0  # Optimizado: intervalos más largos para mejor rendimiento
    ...
    def create_dashboard_tab(self):
        ...
        self.create_metric_card(top_metrics, "🚨 Amenazas", "total_threats", "0", self.colors['error'])
        self.create_metric_card(top_metrics, "🔍 Únicas", "unique_threats", "0", self.colors['warning'])
        self.create_metric_card(top_metrics, "⏱️ Tiempo", "uptime", "00:00:00", self.colors['text_primary'])
        self.create_metric_card(top_metrics, "🔌 Plugins", "active_plugins", "0", self.colors['success'])
        self.create_metric_card(top_metrics, "🛡️ Filtrados", "filtered_events", "0", self.colors['info'])
    ...
    def create_alerts_tab(self):
        ...
        self.threat_context_menu.add_command(label="✅ Marcar como Seguro", command=self.mark_as_safe)
        self.threat_context_menu.add_command(label="🔒 Poner en Cuarentena", command=self.quarantine_item)
        self.threat_context_menu.add_command(label="📁 Abrir Ubicación", command=self.open_file_location)
        self.threat_context_menu.add_command(label="ℹ️ Ver Detalles", command=self.show_threat_details)
        self.threat_context_menu.add_command(label="🚫 Añadir a Lista Blanca", command=self.add_to_whitelist)
    ...
    def create_stats_tab(self):
        ...
        self.create_metric_card_compact(metrics_grid, "🔍 Det/min", "detection_rate_small", "0.0", self.colors['info'], 0)
        self.create_metric_card_compact(metrics_grid, "💾 RAM", "memory_usage_small", "0MB", self.colors['warning'], 1)
        self.create_metric_card_compact(metrics_grid, "⚡ CPU", "cpu_usage_small", "0%", self.colors['error'], 2)
        self.create_metric_card_compact(metrics_grid, "🌐 Filtro", "filter_efficiency_small", "0%", self.colors['success'], 3)
        ...
        self.create_metric_card_large(metrics_grid, "🔍 Detecciones/min", "detection_rate", "0.0", self.colors['info'], 0, 0)
        self.create_metric_card_large(metrics_grid, "� Memoria UI", "memory_usage", "0 MB", self.colors['warning'], 0, 1)
        self.create_metric_card_large(metrics_grid, "⚡ CPU Engine", "cpu_usage", "0%", self.colors['error'], 1, 0)
        self.create_metric_card_large(metrics_grid, "🌐 Filtrado", "filter_efficiency", "0%", self.colors['success'], 1, 1)
        ...
        try:
            ...
            if hasattr(self, 'threat_aggregator'):
                ...
                if threats:
                    ...
                    print(f"📈 Datos reales - Tipos: {type_distribution}, Procesos: {len(sorted_processes)}")
                else:
    ...
    def update_realtime_metrics(self):
        ...
        try:
            ...
            if hasattr(self, 'start_time') and self.start_time and total_events > 0:
                runtime_minutes = max(0.1, (time.time() - self.start_time) / 60)  # Mínimo 0.1 min
            ...
            if total_events > 0:
                ...
                filter_efficiency = 100 - unique_ratio  # Mientras más duplicados filtra, más eficiente
            ...
            if self._metrics_print_counter % 4 == 0:
                print(f"📊 Métricas - Det/min: {detection_rate_val}, RAM: {memory_val}, CPU: {cpu_val}, Filtro: {filter_efficiency_val}")
    ...
    def start_protection(self):
        ...
        current_time = time.time()
        if current_time - self.last_update_time < 5.0:  # Mínimo 5 segundos entre inicios
        ...
        try:
            ...
            with self.engine:
                ...
                if self.engine.start_system(categories):
                    ...
                    self.data_queue.put(('error', 'Error iniciando el motor de protección'))
        except Exception as e:
    ...
    def process_engine_data(self):
        ...
        try:
            ...
            while processed_count < 10 and (time.time() - start_time) < 0.1:  # Máximo 100ms
                try:
...
"""
    ...
    def update_threat_display(self):
        ...
        try:
            ...
            print(f"🔍 Actualizando display: {len(aggregated_threats)} amenazas agregadas")  # Debug
            ...
            print(f"📊 Estadísticas: {total_events} eventos totales, {unique_events} únicos")  # Debug
...
"""
            ...
            if hasattr(self, 'top_processes_text'):
                ...
                for i, (process, count) in enumerate(top_processes, 1):
                    processes_text += f"{i:2d}. {process:<20} → {count:>6} detecciones\n"
    ...
    def quarantine_item(self):
        ...
        if selection:
            ...
            if response:
                ...
                self.threats_tree.item(selection[0], values=item['values'][:-1] + ["🔒 Cuarentena"])
                self.add_smart_log_entry("WARNING", f"✅ Puesto en cuarentena: {process}")
                ...
                if process_path and os.path.exists(process_path):
                    ...
                    for path in common_paths:
                        if os.path.exists(path):
                            ...
                            self.add_smart_log_entry("INFO", f"📁 Ubicación encontrada: {path}")
                            found = True
                            break
                    ...
                    if not found:
                        ...
                        self.add_smart_log_entry("WARNING", f"Ubicación no encontrada: {process}")
    ...
    def apply_preset(self, preset_name):
        ...
        if preset_name in presets and hasattr(self, 'config_vars'):
            ...
            self.on_config_change()
            self.add_smart_log_entry("INFO", f"Preset {preset_name} aplicado - Configuración actualizada")
    ...
    def apply_configuration(self):
        ...
        if self.is_protection_active:
            self.add_smart_log_entry("INFO", "Aplicando nueva configuración al sistema activo...")
            ...
            messagebox.showinfo("Configuración", "Configuración aplicada al sistema activo")
        else:
            self.add_smart_log_entry("WARNING", "La configuración se aplicará al iniciar la protección")
    
    def reset_to_defaults(self):
        """Restaura configuración por defecto expandida"""
        if not hasattr(self, 'config_vars') or not self.config_vars:
            self.add_smart_log_entry("WARNING", "Variables de configuración no inicializadas")
            return
        ...
        self.on_config_change()
        self.add_smart_log_entry("INFO", "Configuración restaurada a valores por defecto")
            ...
            def update_widget_colors(widget):
                try:
                    ...
                    if widget_class == 'Frame':
                        ...
                        if any(preset in str(button_text) for preset in ['Básico', 'Estándar', 'Avanzado', 'Paranoid']):
    ...
    def export_configuration(self):
        ...
        if filename:
            try:
                ...
                self.add_smart_log_entry("INFO", f"Configuración exportada a {filename}")
                messagebox.showinfo("Exportar", f"Configuración exportada correctamente a:\n{filename}")
            except Exception as e:
                self.add_smart_log_entry("ERROR", f"Error exportando configuración: {e}")
        ...
        if filename:
            try:
                ...
                if hasattr(self, 'config_vars'):
                    ...
                    self.on_config_change()
                    self.add_smart_log_entry("INFO", f"Configuración importada: {applied_count} valores aplicados")
                    messagebox.showinfo("Importar", f"Configuración importada correctamente.\n{applied_count} valores aplicados.")
                
            except Exception as e:
                self.add_smart_log_entry("ERROR", f"Error importando configuración: {e}")
        ...
        try:
            ...
            if hasattr(self, '_last_stats_update'):
                if time.time() - self._last_stats_update > 10:
                    ...
                    if hasattr(self, 'current_tab') and self.current_tab.get() == "Estadísticas":
                        self.update_statistics_tab()
    ...
    def on_closing(self):
        ...
        if self.is_protection_active:
            if messagebox.askyesno("Confirmar", "La protección está activa. ¿Cerrar de todas formas?"):
                self.stop_protection()
        ...
        def generate_data():
            try:
                ...
                for i in range(20):  # 20 detecciones para testing
                    ...
                    print(f"🔍 Generada amenaza de prueba #{i+1}: {selected_threat['type']} - {selected_threat['process']}")
